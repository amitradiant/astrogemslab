			...
			if ( ! $currency ) {
				if ( $tab === 'orders' ) {
					if ( ! $report || $report === 'sales_by_date' || apply_filters( 'wmc_admin_reports', false, $report ) ) {
						add_filter( 'woocommerce_reports_get_order_report_data_args', array( $this, 'add_return_order_id' ) );
						add_filter( 'woocommerce_reports_get_order_report_data', array( $this, 'woocommerce_reports_get_order_report_data_1' ), 10, 2 );
						...
						add_filter( 'woocommerce_reports_get_order_report_data', array( $this, 'woocommerce_reports_get_order_report_data' ), 10, 2 );

						if ( count( $this->currencies ) ) {
							foreach ( $this->currencies as $currency_code => $currency_data ) {
								$this->currency = $currency_code;
								add_filter( 'woocommerce_reports_get_order_report_data_args', array( $this, 'report_by_currency_for_all' ) );
								...
								remove_filter( 'woocommerce_reports_get_order_report_data_args', array( $this, 'report_by_currency_for_all' ) );
							}
						}
