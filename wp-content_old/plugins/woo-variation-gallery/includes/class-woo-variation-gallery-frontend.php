...
if ( ! class_exists( 'Woo_Variation_Gallery_Frontend' ) ):
	...
	class Woo_Variation_Gallery_Frontend {
		...
		public function get_embed_url( $main_link ) {
			...
			$re    = '@https?://(www.)?(youtu\.be|youtube\.com)/(watch\?v=)?([^&?]+).?@';
			$subst = 'https://www.youtube.com/embed/$4?feature=oembed&enablejsapi=1&';

			$link = preg_replace( $re, $subst, $main_link, 1 );

			// Vimeo
			// $re    = '@https?://(www.)?vimeo.com/([^/]+)@';
			//$re    = '@https?://(www.|player.)?vimeo.com/(video/)?([^\?]\d+)([\?.+])?@';
			$re    = '@https?://(www.|player.)?vimeo.com/(video/)?([^&?]+).?@';
			$subst = 'https://player.vimeo.com/video/$3?api=1&';
